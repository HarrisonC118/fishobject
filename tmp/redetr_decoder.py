...
from src.nn.mymodule.ASSM import ModifiedASSM
...
class MSDeformableAttention(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        scaling = torch.arange(1, self.num_points + 1,
    ...
    def _get_sampling_locations(reference_points: torch.Tensor,
        ...
        if ref_dim == 2:
            ...
            raise ValueError(
                f"Last dim of reference_points must be 2 or 4, but got {ref_dim}")
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        # ASSM module for small target low confidence tokens (optional)
        self.use_assm = use_assm
        if use_assm:
            self.assm = ModifiedASSM(
                dim=d_model,
                d_state=assm_d_state,
                num_tokens=assm_num_tokens,
                inner_rank=assm_inner_rank
            )
            self.assm_token = nn.Embedding(assm_inner_rank, assm_d_state)
    ...
    def _compute_small_target_mask(self, reference_points, score_logits):
        ...
        计算小目标低置信度token的mask
        Args:
            reference_points: [B, num_queries, num_levels, 2] 或 [B, num_queries, 2]
            score_logits: [B, num_queries, num_classes] - 当前token的分类logits
        Returns:
            mask: [B, num_queries] - True表示小目标低置信度token
        """
        if reference_points is None or score_logits is None:
            # 如果没有提供必要信息，返回全False的mask（使用原始cross attention）
            return torch.zeros(reference_points.shape[:2], dtype=torch.bool, device=reference_points.device)

        # 处理reference_points的维度
        if reference_points.dim() == 4:  # [B, num_queries, num_levels, 2]
            # 取第一个level的坐标，并假设是[cx, cy]格式，需要从别处获取w,h
            ref_points = reference_points[:, :, 0, :]  # [B, num_queries, 2]
            # 对于[cx, cy]格式，我们无法直接获得面积，设为较小值以便能够触发小目标检测
            areas = torch.ones_like(ref_points[:, :, 0]) * 0.005
        # [B, num_queries, 4]
        elif reference_points.dim() == 3 and reference_points.shape[-1] == 4:
            # [cx, cy, w, h] 格式
            areas = reference_points[:, :, 2] * \
                reference_points[:, :, 3]  # w * h
        else:  # [B, num_queries, 2] 格式
            # [cx, cy] 格式，设为较小值
    ...
    def forward(self,
        ...
        # If ASSM is enabled, selectively apply it to small target low confidence tokens
        if self.use_assm and score_logits is not None:
            # 计算小目标低置信度token的mask
            small_target_mask = self._compute_small_target_mask(
                reference_points, score_logits)

            # ASSM处理 (用于小目标低置信token)
            assm_output = self.assm(self.with_pos_embed(
                tgt, query_pos_embed), self.assm_token)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # 获取当前的score logits用于小目标检测
            current_score_logits = score_head[i](output) if i > 0 else None

            output = layer(output, ref_points_input, memory,
                           memory_spatial_shapes, memory_level_start_index,
                           attn_mask, memory_mask, query_pos_embed, current_score_logits)

            inter_ref_bbox = F.sigmoid(bbox_head[i](
                output) + inverse_sigmoid(ref_points_detach))
            ...
            if self.training:
                ...
                if i == 0:
                    ...
                    dec_out_bboxes.append(F.sigmoid(bbox_head[i](
                        output) + inverse_sigmoid(ref_points)))
...
class RTDETRTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        decoder_layer = TransformerDecoderLayer(
            ...
            use_assm=use_assm,
            assm_d_state=assm_d_state,
            assm_num_tokens=assm_num_tokens,
            assm_inner_rank=assm_inner_rank
        )
            ...
            self.input_proj.append(
                nn.Sequential(OrderedDict([
                    ('conv', nn.Conv2d(in_channels,
                     self.hidden_dim, 3, 2, padding=1, bias=False)),
                    ('norm', nn.BatchNorm2d(self.hidden_dim))])
                )
            )
    ...
    def _generate_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            anchors.append(torch.concat(
                [grid_xy, wh], -1).reshape(-1, h * w, 4))

        anchors = torch.concat(anchors, 1).to(device)
        valid_mask = ((anchors > self.eps) * (anchors <
                      1 - self.eps)).all(-1, keepdim=True)
    ...
    def forward(self, feats, targets=None):
        ...
        if self.training and self.aux_loss:
            ...
            out['aux_outputs'].extend(self._set_aux_loss(
