...
from src.misc.tensorboard_logger import TensorboardLogger
...
class BaseSolver(object):
    ...
    def setup(self, ):
        ...
        # Define output_dir first as TensorboardLogger might use it
        self.output_dir = Path(cfg.output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)

        # Initialize TensorBoard logger
        # Use tensorboard_logger from config if provided, otherwise create a new one
        if hasattr(cfg, 'tensorboard_logger') and cfg.tensorboard_logger is not None:
            self.tensorboard_logger = cfg.tensorboard_logger
        else:
            self.tensorboard_logger = TensorboardLogger(log_dir=self.output_dir) # Now self.output_dir is defined
        # 默认就是cfg.model.to(device)
